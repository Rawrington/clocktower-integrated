...
const DraggableNote = forwardRef(({ id }, ref) => {
  const { reminder, text, position } = useSelector(state => getNoteById(state, id));
  ...
  function findClosestPlayer(curX, curY) {
    ...
    playerListPos.forEach((player, i) => {
      const distSqr = ((player.x - curX) * (player.x - curX)) + ((player.y - curY) * (player.y - curY));
  ...
  useEffect(() => {
    findClosestPlayer(position.x, position.y);
  }, [position.x, position.y, playerListPos.length > 0 && playerListPos[0].x])
  ...
  useEffect(() => {
    function handleResize() {
      ...
      dispatch(updateNotePos({
        id: id,
        x: centerX + ((((position.x + 3.25 * oldVminApprox) - dimensions.x) / oldVminApprox) * vminApprox) - 3.25 * vminApprox,
        y: centerY + ((((position.y + 3.25 * oldVminApprox) - dimensions.y) / oldVminApprox) * vminApprox) - 3.25 * vminApprox,
      }));
  ...
  }, [ref, ref.current, resizing, dimensions.x, dimensions.y, dimensions.width, dimensions.height, position.x, position.y]);
  ...
  const forcedPosition = resizing ? position : null;
